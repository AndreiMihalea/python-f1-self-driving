...
from data_loading.F1DatasetSimple import F1Dataset, ToTensor, Crop, Normalize
...
def normalize_weights(weights):
    return ((weights.max() + 1) - weights) / weights.sum()
...
dataset = F1Dataset(base + 'driving_log.csv', base, future_frame=0, augment=False,
                    transform=transforms.Compose([Crop((0.4, 0.39, 0.13, 0.13))]))
...
plt.plot(weights_acc)
plt.plot(weights_brake)
plt.plot(weights_steer)
plt.show()
...
dataset = F1Dataset(base + 'driving_log.csv', base, future_frame=2,
...
train_loader = DataLoader(dataset, batch_size=512, num_workers=0, sampler=train_sampler)
valid_loader = DataLoader(dataset, batch_size=512, num_workers=0, sampler=valid_sampler)
...
def train(epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        if i % print_each == print_each - 1:
            ...
            running_loss = 0.0
    torch.save(model.state_dict(), 'C:\\Users\\Andrei\\workspace\\Robotics\\checkpoints\\checkpoint_train_resnet_%d' % (epoch))
